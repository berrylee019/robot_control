...
if uploaded_file is not None:
    # 형님이 파일을 넣으면 그 데이터를 사용
    df = pd.read_csv(uploaded_file)
    st.sidebar.success("✅ 시연용 데이터 로드 완료!")
else:
    # 파일이 없으면 기본 가상 데이터를 사용
    df = get_robot_data()
...
with kpi1:
    ...
    st.metric("📊 전체 가동률", f"{int((availability/len(df))*100)}%", delta="1.2%", help="현재 가동 중인 로봇의 비율")

with kpi2:
    # 예지 보전을 통해 방어한 다운타임 비용 계산
    danger_count = (df['Failure Risk (%)'] > target_risk).sum()
    saved_cost = danger_count * 1250000 
    st.metric("💰 예지보전 절감액", f"₩{saved_cost:,}", delta=f"보호 대상 {danger_count}대", delta_color="normal")

with kpi3:
    st.metric("🛠️ 긴급 점검 대상", f"{danger_count} 대", delta="신규 발생", delta_color="inverse")
